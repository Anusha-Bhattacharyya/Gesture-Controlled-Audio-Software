...
with sd.OutputStream(device=output_device, samplerate=samplerate, channels=2, callback=callback):
    ...
    # Import the necessary variables
        ...
        if gesture_name == "ThumbsUp":
            gain_plugin.gain_db+=2
            compressor_plugin.threshold_db+=1
            compressor_plugin.ratio+=1
            print("ThumbsUp")
            ...
            if delay_plugin.mix < 1:
                reverb_plugin.wet_level += 10
                reverb_plugin.room_size += 1
            print("OpenHand")
            if os.path.exists(SAMPLE_PATH):
                # with suppress(BaseException):
                try:
            ...
            delay_plugin.delay_seconds += 0.5
            if delay_plugin.mix < 1:
                delay_plugin.mix += 0.1
            print("ClosedFist")
            if os.path.exists(SAMPLE_PATH):
                # with suppress(BaseException):
                try:
            ...
            high_pass_plugin.cutoff_frequency_hz += 500 
            ...
            low_pass_plugin.cutoff_frequency_hz -= 500 
            ...
            pitch_shift_plugin.semitones = 0
            print("PointingLeft")
            if os.path.exists(SAMPLE_PATH):
                # with suppress(BaseException):
                try:
